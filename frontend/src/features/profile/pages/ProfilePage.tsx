...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    setLocation(user.profile.location_name ?? '');
  ...
  if (!user) return <EmptyState title="Necesitas iniciar sesión" description="Accede para ver y editar tu perfil." action={{ label: 'Ir a login', onClick: () => navigate('/login') }} />;

  const handleSave = async () => {
    setSaving(true);
    try {
      await api.patch(endpoints.myProfile, { bio_extended: bio, location_name: location, instagram, website });
      ...
      <section className="grid gap-8 xl:grid-cols-[1.2fr_0.45fr] xl:items-start">
        <div className="space-y-8">
            ...
            <div className="flex flex-col gap-6 md:flex-row md:items-end">
              <div className="relative">
                <div className="h-32 w-32 overflow-hidden rounded-full border-4 border-primary-container p-1 md:h-40 md:w-40">
                  <img src={user.avatar} alt={user.full_name} className="h-full w-full rounded-full object-cover" />
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-3 text-sm text-on-surface-variant">
                  {location && <span className="inline-flex items-center gap-2 font-label text-[10px] uppercase tracking-[0.16em]"><MapPin size={14} className="text-primary" /> {location}</span>}
                </div>
          ...
          <section className="grid gap-px overflow-hidden rounded-[1.6rem] border border-outline-variant/10 bg-outline-variant/10 md:grid-cols-3">
            <div className="bg-surface-container px-6 py-7 text-center">
              <p className="font-headline text-4xl font-black text-primary">{user.profile.total_activities}</p>
              <p className="mt-2 font-label text-[10px] uppercase tracking-[0.18em] text-on-surface-variant">Activities</p>
            </div>
            <div className="bg-surface-container px-6 py-7 text-center">
              <p className="font-headline text-4xl font-black text-primary">{user.profile.total_km} km</p>
              <p className="mt-2 font-label text-[10px] uppercase tracking-[0.18em] text-on-surface-variant">Travelled</p>
            </div>
            <div className="bg-surface-container px-6 py-7 text-center">
              <p className="font-headline text-4xl font-black text-primary">{user.profile.total_people_met}</p>
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-3">
              {user.profile.badges.length > 0 ? user.profile.badges.map((badge) => (
        ...
        <div className="space-y-6">
          <section className="editorial-card rounded-[1.8rem] border-l-4 border-primary px-6 py-6 md:px-8">
            ...
            <p className="mt-3 font-headline text-4xl font-black text-on-surface">{user.profile.avg_rating ?? 5}</p>
          </section>
          ...
          <section className="editorial-card rounded-[2rem] px-6 py-6 md:px-8 md:py-8 space-y-6">
            ...
            {editing ? (
              <div className="space-y-4">
                ...
                <input className="editorial-input" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Ubicación" />
          ...
          <section className="editorial-card rounded-[2rem] px-6 py-6 md:px-8 md:py-8">
            ...
            <div className="mt-4 space-y-4 text-sm text-on-surface-variant">
              <div className="flex items-center gap-3"><MapPin size={16} className="text-primary" /> {location || 'Sin ubicación definida'}</div>
