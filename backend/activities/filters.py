...
class ActivityFilter(django_filters.FilterSet):
    ...
    def filter_by_distance(self, queryset, name, value):
        ...
        When running on the PostGIS backend, uses raw PostGIS distance SQL for
        ...
        if connection.settings_dict.get("ENGINE") == POSTGIS_ENGINE:
    ...
    def _filter_postgis(queryset, lat, lng, radius_km):
        """PostGIS dwithin()/distance SQL for precise geodetic filtering."""
    ...
    def _filter_bounding_box(queryset, lat, lng, radius_km):
        ...
        lng_offset = radius_km / (111.0 * max(math.cos(math.radians(lat)), 0.001))
